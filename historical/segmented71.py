...
prompts at client concurrency 71. Timings are client wall intervals and server
request-event intervals, NOT CUDA kernel profiling. Server limits changed to context=6144 and max-num-seqs=71; model and prompts unchanged.
"""
...
assert len(CATS)==71
CLIENT=httpx.Client(base_url='http://127.0.0.1:8000',timeout=httpx.Timeout(120,connect=10),limits=httpx.Limits(max_connections=80,max_keepalive_connections=80),trust_env=False)
...
def interpret(d,expected):
 ...
 # Retain preemptions in measured results; do not silently discard overloaded trials.
 return out
